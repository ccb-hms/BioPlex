...
```{r}
valid.uids <- AnnotationDbi::keys(orgdb, keytype = "UNIPROT")
"Q8N7W2" %in% valid.uids
"Q8N7W2-1" %in% valid.uids
"Q8N7W2-2" %in% valid.uids
```

Update: I think we need to use here the Ensembl mapping instead
...
Extract CCLE transcriptome data and proteome data for HCT116: 

```{r ccle-trans-prot, message = FALSE}
hct.trans <- assay(ccle)[,ccle$cell_line == "HCT 116"]
names(hct.trans) <- AnnotationDbi::mapIds(orgdb, 
                      keys = names(hct.trans), 
                      keytype = "ENSEMBL",
                      column = "ENTREZID")
hct.prot <- assay(ccle.prot)[,"HCT116"]
names(hct.prot) <- rowData(ccle.prot)$ENTREZID
```

Correlate measurements for ENTREZ gene IDs present in both modalities:
```{r}
length(hct.prot)
length(hct.trans)
table(names(hct.prot) %in% names(hct.trans))
isect <- intersect(names(hct.prot), names(hct.trans))
hct.prot <- hct.prot[isect]
hct.trans <- hct.trans[isect]
```
